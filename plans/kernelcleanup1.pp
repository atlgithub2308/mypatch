plan mypatch::kernelcleanup1(
  ...
  $boot_pre = run_command('df -h /boot', $targets)

  # 2) List available kernels (pre)
  $kernels_pre = run_command('rpm -qa kernel', $targets)
  ...
  $cleanup = run_command($cleanup_command, $targets, '_catch_errors' => true)

  # 4) Pause for 15 seconds
  $sleep = run_command('sleep 15', $targets)

  # 5) Check /boot disk space (post)
  $boot_post = run_command('df -h /boot', $targets)

  # 6) List available kernels (post)
  $kernels_post = run_command('rpm -qa kernel', $targets)

  return {
    'boot_pre'     => $boot_pre,
    'kernels_pre'  => $kernels_pre,
    'cleanup'      => $cleanup,
    'sleep'        => $sleep,
    'boot_post'    => $boot_post,
